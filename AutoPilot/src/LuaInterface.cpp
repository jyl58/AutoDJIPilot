...
int LuaInterface::LuaTakeoff(lua_State* lua){
	...
	if(!_flight_core->djiTakeoff()){
		DWAR("Runing takeoff err.",luaSocketPrintFd);
...
int LuaInterface::Lualand(lua_State* lua){
	...
	if(!_flight_core->djiLanding()){
		DWAR("Runing land err. ",luaSocketPrintFd);
		lua_pushboolean(lua,LUA_FAIL);
		lua_error(lua); // interrupt run the script
		return LUA_FAIL;
	}
...
int LuaInterface::LuaGoHome(lua_State* lua){
	...
	if(!_flight_core->djiGoHome()){
		DWAR("Runing go home err. ",luaSocketPrintFd);
		lua_pushboolean(lua,LUA_FAIL);
		lua_error(lua); // interrupt run the script
		return LUA_FAIL;
	}
...
int LuaInterface::LuaFlyByGPS(lua_State* lua){
	if(lua_gettop(lua) != 2){
		DWAR("Param Need Latitude and longitude. ",luaSocketPrintFd);
	...
	if((target_lat> 90 || target_lat < -90) || (target_lon > 180 || target_lon < -180)){
		DWAR("Latitude and longitude is out the range.",luaSocketPrintFd);
	...
	if(!_flight_core->djiMoveByGPS(target_lat,target_lon)){
		DWAR("Runing move by gps err.",luaSocketPrintFd);
		lua_pushboolean(lua,LUA_FAIL);
...
int LuaInterface::LuaFlyByVelocity(lua_State* lua){
	if(lua_gettop(lua) != 3){
		DWAR("Need vx,vy,vz.",luaSocketPrintFd);
		lua_error(lua); // interrupt run the script
		return LUA_FAIL;
	}
	...
	if(!_flight_core->djiMoveByVelocity(vx,vy,vz)){
		DWAR("Runing move by velocity err.",luaSocketPrintFd);
		lua_pushboolean(lua,LUA_FAIL);
		lua_error(lua); // interrupt run the script
		return LUA_FAIL;
	}
...
int LuaInterface::LuaFlyByBearingAndDistance(lua_State* lua){
	if(lua_gettop(lua) != 2){
		DWAR("Need a bearing(deg)  and a distance(m) argument.",luaSocketPrintFd);
		lua_error(lua); // interrupt run the script
		return LUA_FAIL;
	}
	...
	if(std::fabs(bearing)>180){
		DWAR("Target bearing is out the range.",luaSocketPrintFd);
		lua_error(lua); // interrupt run the script
		return LUA_FAIL;
	}
	if(!_flight_core->djiMoveByBearingAndDistance(bearing,distance)){
		DWAR("Running move by bearing and distance err.",luaSocketPrintFd);
		lua_error(lua); // interrupt run the script
		return LUA_FAIL;
	}
	lua_pushboolean(lua,LUA_SUCESS);
...
int LuaInterface::LuaTurnHead(lua_State* lua){
	if(lua_gettop(lua) != 1){
		DWAR("Param need the target head(deg).",luaSocketPrintFd);
		lua_error(lua); // interrupt run the script
		return LUA_FAIL;
	}
	...
	if(std::fabs(target_head) > 180){
		DWAR("Target head is out the range",luaSocketPrintFd);
		lua_error(lua); // interrupt run the script
		return LUA_FAIL;
	}
	if(!_flight_core->djiTurnHead(target_head)){
		DWAR("Runing turn head err. ",luaSocketPrintFd);
		lua_pushboolean(lua,LUA_FAIL);
		lua_error(lua); // interrupt run the script
		return LUA_FAIL;
	}
...
int LuaInterface::LuaClimbTo(lua_State* lua){
	if(lua_gettop(lua) != 1){
		DWAR("Param need the target alt(m)",luaSocketPrintFd);
		lua_error(lua); // interrupt run the script
		return LUA_FAIL;
	}
	//get param from lua 
	...
	if(!_flight_core->djiMoveZToTarget(target_alt)){
		DWAR("Runing climb err. ",luaSocketPrintFd);
		lua_pushboolean(lua,LUA_FAIL);
		lua_error(lua); // interrupt run the script
		return LUA_FAIL;
	}
...
int LuaInterface::LuaClimbBy(lua_State* lua){
	...
	if(!_flight_core->djiMoveZByOffset(target_alt)){
		DWAR("Runing climb by err. ",luaSocketPrintFd);
		lua_pushboolean(lua,LUA_FAIL);
		lua_error(lua); // interrupt run the script
		return LUA_FAIL;
	}
...
int LuaInterface::LuaDelay(lua_State* lua){
	if(lua_gettop(lua) != 1){
		DWAR("LuaDelay need a time argument (ms). ",luaSocketPrintFd);
		lua_error(lua); // interrupt run the script
		return LUA_FAIL;
	}
...
int LuaInterface::LuaCall(lua_State* lua){
	...
	if(param_count<1){
		DWAR("LuaCall  need at least one param for function name",luaSocketPrintFd);
		lua_error(lua); // interrupt run the script
		return LUA_FAIL;
	}
...
int LuaInterface::LuaShootPhoto(lua_State* lua){
	...
	if(!_flight_core->djiShootPhoto()){
		DWAR("Shoot photo err.",luaSocketPrintFd);
		lua_error(lua); // interrupt run the script
		return LUA_FAIL;
	}
	lua_pushboolean(lua, LUA_SUCESS);
...
int LuaInterface::LuaVideoStart(lua_State* lua){
	...
	if(!_flight_core->djiVideoStart()){
		DWAR("Start video err.",luaSocketPrintFd);
		lua_error(lua); // interrupt run the script
		return LUA_FAIL;
	}
	lua_pushboolean(lua, LUA_SUCESS);
...
int LuaInterface::LuaVideoStop(lua_State* lua){
	...
	if(!_flight_core->djiVideoStop()){
		DWAR("Stop video err.",luaSocketPrintFd);
		lua_error(lua); // interrupt run the script
		return LUA_FAIL;
	}
	return LUA_SUCESS;
...
int LuaInterface::LuaCameraZoom(lua_State* lua){
	if(lua_gettop(lua) != 2){
		DWAR("Need mode and value 2 argument (deg). ",luaSocketPrintFd);
		lua_error(lua); // interrupt run the script
		return LUA_FAIL;
	}
	...
	if(!_flight_core->djiCameraZoom(mode,value)){
		DWAR("Camera zoom err. ",luaSocketPrintFd);
		lua_error(lua); // interrupt run the script
		return LUA_FAIL;
	}
	return LUA_SUCESS;
...
LuaInterface::LuaSetGimbalAngle(lua_State* lua){
	if(lua_gettop(lua) != 3){
		DWAR("Need roll,pitch,yaw 3 argument (deg). ",luaSocketPrintFd);
		lua_error(lua); // interrupt run the script
		return LUA_FAIL;
	}
	...
	if(fabs(yaw_deg)>320){
		DWAR("desire Yaw angle is out of limit",luaSocketPrintFd);
		yaw_deg=yaw_deg>0? 320:-320;
	}
	if( fabs(roll_deg)>35){
		DWAR("desire Roll angle is out of limit",luaSocketPrintFd);
		yaw_deg=roll_deg>0? 35:-35;
	}
	if(pitch_deg<-90 || pitch_deg>30){
		DWAR("desire Pitch angle is out of limit",luaSocketPrintFd);
		yaw_deg=pitch_deg>0? 30:-90;
	}
	if(!_flight_core->djiSetGimbalAngle(roll_deg,pitch_deg,yaw_deg)){
		DWAR("Set gimbal angle err.",luaSocketPrintFd);
		lua_error(lua); // interrupt run the script
		return LUA_FAIL;	
	}
	return LUA_SUCESS;
...
LuaInterface::LuaTestMotor(lua_State* lua){
	...
	if(!_flight_core->djiArmMotor()){
		DWAR("Start motor err.",luaSocketPrintFd);
		lua_error(lua); // interrupt run the script
		return LUA_FAIL;
	}
	int count=0;
	...
	if(!_flight_core->djiDisarmMotor()){
		DWAR("Stop motor err.",luaSocketPrintFd);
		lua_error(lua); // interrupt run the script
		return LUA_FAIL;
	}
